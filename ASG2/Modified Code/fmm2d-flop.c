...
/*----------------------------------------------------------------------------*/
  ...
  printf("===== FMM & direct CPU times = %e %e =====\n",
    tfmm,tdirect);
  printf("===== FMM & direct floating-point operation counts = %e %e =====\n",
    ffmm,fdirect);
  printf("===== FMM & direct Gflop/s = %e %e =====\n",
    ffmm/tfmm/1.0e9,fdirect/tdirect/1.0e9);
...
------------------------------------------------------------------------------*/
  ...
  for (j=0; j<Npar-1; j++)
    ...
    fdirect += (10.0 + FSQRT + 2.0 * FLOG) * Npar * (Npar - 1.0) / 2.0;
